...
export async function generatePDFSummary(
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    // Separator
    doc.moveTo(50, y).lineTo(PDF.WIDTH - 50, y).lineWidth(1).strokeColor("#CCCCCC").stroke();
    y += 20;
    ...
    // Separator
    doc.moveTo(50, y).lineTo(PDF.WIDTH - 50, y).lineWidth(0.5).strokeColor("#CCCCCC").stroke();
    y += 15;

    // Table header
    const colX = { num: 50, client: 130, date: 260, subtotal: 330, vat: 400, total: 460, status: 520 };
    const colW = { num: 75, client: 125, date: 65, subtotal: 65, vat: 55, total: 55, status: 30 };
    ...
    doc.text("Subtotal", colX.subtotal, y + 6, { width: colW.subtotal, align: "right" });
    ...
    for (const inv of invoices) {
      ...
      doc.text(inv.clientName, colX.client, y + 4, { width: colW.client, ellipsis: true });
      doc.text(inv.invoiceDate, colX.date, y + 4, { width: colW.date });
      doc.text(formatCurrency(inv.subtotal), colX.subtotal, y + 4, { width: colW.subtotal, align: "right" });
      doc.text(formatCurrency(inv.vatAmount), colX.vat, y + 4, { width: colW.vat, align: "right" });
      doc.text(formatCurrency(inv.total), colX.total, y + 4, { width: colW.total, align: "right" });
